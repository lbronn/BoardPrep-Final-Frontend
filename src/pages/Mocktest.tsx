...
const Mocktest: React.FC = () => {
  ...
  const [mocktestDetail, setMocktestDetails] = useState<MocktestDetail | null>(
    null
  );
  ...
  const { course_id, classID } = useParams<{
    course_id: string;
    classID: string;
  }>();
  ...
  useEffect(() => {
    if (classID) {
      axiosInstance
        ...
        .catch((error) => {
          console.error(
            "There was an error fetching the mock test details.",
            error
          );
        });
    }
  ...
  useEffect(() => {
    if (mocktest_id) {
      axiosInstance
        .get(`/questions/?mocktest_id=${mocktest_id}`)
        .then((response) => {
          if (response.data.length > 0) {
            const fetchedMocktest = response.data[0];
